...
async def save_uploaded_file(request: Request, directory: str = "files") -> str:
    ...
    if not isinstance(upload, (UploadFile, StarletteUploadFile)):
        raise HTTPException(status_code=400, detail="Invalid payload for 'file'")
...
async def make_twilio_call(phone_number: str) -> TwilioCallResult:
    ...
    # Enable dual-channel recording and provide a callback for when recording is ready.
    # Twilio will POST recording events to /recording_callback on our server.
    recording_callback_url = f"{local_server_url}/recording_callback"
...
def read_excel_file(file_path: str) -> list[str]:
    ...
    if "PHONE_NUMBER" not in df.columns:
        raise KeyError("PHONE_NUMBER column missing in Excel file")

    phone_numbers = df["PHONE_NUMBER"].dropna().astype(str).str.strip()
    return [number for number in phone_numbers if number]
